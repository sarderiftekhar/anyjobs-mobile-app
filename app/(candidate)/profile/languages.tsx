import { useState } from "react";
import { View, Text, ScrollView, TouchableOpacity, Alert } from "react-native";
import { router } from "expo-router";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { Ionicons } from "@expo/vector-icons";
import { Button, Input, Card, EmptyState } from "../../../src/components/ui";
import { ScreenHeader, ChipSelect } from "../../../src/components/form";
import { useAuthStore } from "../../../src/stores/authStore";
import { useUpdateLanguages } from "../../../src/hooks/useProfile";
import type { LanguageEntry } from "../../../src/types/user";

const LEVELS = ["Basic", "Conversational", "Professional", "Fluent", "Native"];

export default function LanguagesScreen() {
  const insets = useSafeAreaInsets();
  const existing = useAuthStore((s) => s.user?.profile?.languages) ?? [];
  const [items, setItems] = useState<LanguageEntry[]>(() => existing.map((l) => ({ ...l })));
  const mutation = useUpdateLanguages();

  const update = (i: number, k: keyof LanguageEntry, v: string) =>
    setItems((arr) => arr.map((it, idx) => (idx === i ? { ...it, [k]: v } : it)));
  const add = () => setItems((arr) => [...arr, { name: "", proficiency: "Professional" }]);
  const remove = (i: number) => setItems((arr) => arr.filter((_, idx) => idx !== i));

  const onSave = async () => {
    const cleaned = items.filter((l) => l.name.trim());
    try {
      await mutation.mutateAsync(cleaned);
      router.back();
    } catch (e: any) {
      Alert.alert("Couldn't save", e?.response?.data?.message ?? "Please try again.");
    }
  };

  return (
    <View className="flex-1 bg-background">
      <View style={{ paddingTop: insets.top }}>
        <ScreenHeader title="Languages" />
      </View>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 120 }} keyboardShouldPersistTaps="handled">
        {items.length === 0 ? (
          <EmptyState icon="language-outline" title="No languages yet" description="Add the languages you speak and your proficiency." actionTitle="Add language" onAction={add} />
        ) : (
          items.map((l, i) => (
            <Card key={i} className="mb-3" animated={false}>
              <View className="mb-2 flex-row items-center justify-between">
                <Text className="text-sm font-semibold text-ink-muted">Language {i + 1}</Text>
                <TouchableOpacity onPress={() => remove(i)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                  <Ionicons name="trash-outline" size={18} color="#EF4444" />
                </TouchableOpacity>
              </View>
              <Input label="Language" value={l.name} onChangeText={(v) => update(i, "name", v)} placeholder="English" />
              <Text className="mb-2 text-sm font-medium text-ink">Proficiency</Text>
              <ChipSelect options={LEVELS} value={l.proficiency ? [l.proficiency] : []} onChange={(next) => update(i, "proficiency", next[0] ?? "")} multiple={false} />
            </Card>
          ))
        )}
        {items.length > 0 && (
          <Button title="Add another" variant="outline" icon={<Ionicons name="add" size={18} color="#0064EC" />} onPress={add} />
        )}
      </ScrollView>
      <View className="absolute inset-x-0 bottom-0 border-t border-border bg-surface px-4 pt-3" style={{ paddingBottom: insets.bottom + 12 }}>
        <Button title="Save" loading={mutation.isPending} onPress={onSave} />
      </View>
    </View>
  );
}
